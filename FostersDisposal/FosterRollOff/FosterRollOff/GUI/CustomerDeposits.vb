Public Class CustomerDeposits
    ...
    Private Sub FillList()
        ...
        For Each chrObj As CDeposits In CDeposits.GetDepositList(SelectedRollOff)
    ...
    Private Sub ButtonDelete_Click(sender As Object, e As EventArgs) Handles ButtonDelete.Click
        If DataGridViewCustomerPayments.SelectedRows.Count > 0 Then
            If MessageBox.Show("Are you sure you would like to delete the selected deposit?", "Confirm", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                Dim selectedDGR As DataGridViewRow = DataGridViewCustomerPayments.SelectedRows(0)
                Dim delChargeObj As New CDeposits
    ...
    Private Sub ButtonQuickDeposit_Click(sender As Object, e As EventArgs) Handles ButtonQuickDeposit.Click

        With DataGridViewCustomerPayments
            Dim newPaymentRow As Integer = .Rows.Add
            .Rows(newPaymentRow).Cells("colDescription").Value = "DEPOSIT"
    ...
    Private Sub ButtonRefund_Click(sender As Object, e As EventArgs) Handles ButtonRefund.Click
        ...
        With DataGridViewCustomerPayments
            Dim newPaymentRow As Integer = .Rows.Add
            .Rows(newPaymentRow).Cells("colDescription").Value = "REFUND"
            .Rows(newPaymentRow).Cells("colPaymentDate").Value = String.Format("{0:MM/dd/yyyy}", Date.Today)
            .CurrentCell = .Rows(newPaymentRow).Cells("colPaymentAmount")
            .BeginEdit(True)
        End With
    ...
    Private Sub ButtonPrintReceipt_Click(sender As Object, e As EventArgs) Handles ButtonPrintReceipt.Click
        ...
        If DataGridViewCustomerPayments.SelectedRows.Count > 0 Then
            ...
            if SelectedRollOff.Customer.SendToCollections then
                dim paymentsSinceCollections =
                        CPayments.GetPaymentList(SelectedRollOff).Where(
                            Function(f) f.TransactionDate > SelectedRollOff.Customer.CollectionBalanceAsOf).Sum(
                                function(f) f.Amount)
                frm.Receipt.CurrentBalance = SelectedRollOff.Customer.CurrentBalanceForCollection -
                                             paymentsSinceCollections
            Else
